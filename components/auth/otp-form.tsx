...
export function OtpForm({ initial }: { initial: OtpStatus }) {
  const [verifyState, verify, pending] = useActionState(verifyOtpAction, {});
  ...
  const status = resendState.status ?? initial;
  const expiry = secondsUntil(status.expires_at);
  const wait = secondsUntil(status.resend_at);
  const mm = String(Math.floor(expiry / 60)).padStart(2, "0");
  const ss = String(expiry % 60).padStart(2, "0");

  const errorMsg = verifyState.error || resendState.error;
    ...
    <>
      ...
      <form action={verify} className="mt-6 space-y-4">
        <div className="space-y-1.5">
          ...
          <Input
            className="h-11 text-center font-mono text-xl tracking-[.35em]"
            id="code"
            name="code"
            inputMode="numeric"
            pattern="[0-9]{6}"
        ...
        <div className="flex items-center justify-between text-xs">
          <span className={expiry ? "text-gray-500" : "text-red-600"}>
